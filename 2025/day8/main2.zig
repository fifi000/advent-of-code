...
fn solve(allocator: Allocator, lines: []const String) Int {
    ...
    for (points, 0..) |p1, i| {
        print("processing point {any}/{any}\n", .{ i + 1, points.len });
        for (points[i + 1 ..]) |p2| {
    ...
    var counter: usize = 0;
    while (circuits.items.len != 1) : (counter += 1) {
        print("processing tuple {any}, circuits left: {any}\n", .{ counter + 1, circuits.items.len });
